...
public class MavenJsonCatalogResource {
	...
	@PostConstruct
	public void init() {
		final List<String> recognizedQuarkusVersions = Arrays.asList("", RegistryConstants.QUARKUS_VERSION);

		final PomContentProvider pomProvider = new PomContentProvider();
		final MavenMetadataContentProvider metadataProvider = new MavenMetadataContentProvider();
		final MavenMetadataContentProvider quarkusVersionMetadataProvider = new MavenMetadataContentProvider(recognizedQuarkusVersions);
		...
		}, pomProvider, new NonPlatformExtensionsProvider(recognizedQuarkusVersions), quarkusVersionMetadataProvider));
	...
	private String resolve(UriInfo uriInfo, ArtifactCoords coords,
			ArtifactContentProvider contentProvider, String checksumSuffix) {
